...
module.exports = class AircraftController {
  ...
  static async updateAircraft(req, res) {
    ...
    if (req.files?.image_url_2) {
      image_url_2 = req.files.image_url_2[0].path;
    }
    if (req.files?.image_url_3) {
      image_url_3 = req.files.image_url_3[0].path;
    }
    if (req.files?.image_url_4) {
      image_url_4 = req.files.image_url_4[0].path;
    }
    ...
    try {
      ...
      const response = await AircraftServices.updateAircraft(id, newAircraft);

      return successResponse(
        res,
        201,
        "Updated aircraft successfully",
        response
      );
    } catch (error) {
      return errorResponse(res, 500, "Server Error");
  ...
  static async deleteAircraftById(req, res) {
    try {
      const { id } = req.params;
      const response = await AircraftServices.deleteAircraftById(id);
      return successResponse(res, 200, "Aircraft deleted successfully", response);
    } catch (error) {
      return errorResponse(res, 500, "Server Error");
    }
  }
